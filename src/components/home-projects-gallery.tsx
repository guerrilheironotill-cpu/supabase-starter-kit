...
export function HomeProjectsGallery({ projects }: { projects: HomeProject[] }) {
  ...
  const previous = () =>
    setOpenIndex((value) =>
      value === null ? null : (value - 1 + projects.length) % projects.length,
    );
  const next = () =>
    setOpenIndex((value) => (value === null ? null : (value + 1) % projects.length));
  ...
  return (
    <section className="bg-white pt-12 pb-0 sm:pt-16" aria-labelledby="projects-with-arteno-title">
      ...
      <Carousel
        opts={{ align: "start", loop: projects.length > 6, duration: 35 }}
        className="group/gallery mt-9 w-full"
      >
        <CarouselContent className="ml-0">
          {projects.map((project, index) => (
            <CarouselItem
              key={project.id}
              className="basis-full pl-0 min-[480px]:basis-1/2 md:basis-1/3 lg:basis-1/4 xl:basis-1/5 min-[1440px]:basis-1/6"
            >
              <button
                type="button"
      ...
      {selected && (
        ...
        >
          <button
            ...
            className="absolute right-4 top-4 z-10 rounded-full bg-black/35 p-2 text-white hover:bg-black/60"
          >
            <X className="h-6 w-6" />
          </button>
          {projects.length > 1 && (
            <>
              <button
                ...
                className="absolute left-2 top-1/2 z-10 -translate-y-1/2 rounded-full bg-black/35 p-2 text-white hover:bg-black/60 sm:left-6 sm:p-3"
              >
              ...
              <button
                ...
                className="absolute right-2 top-1/2 z-10 -translate-y-1/2 rounded-full bg-black/35 p-2 text-white hover:bg-black/60 sm:right-6 sm:p-3"
              >
          ...
          <figure className="flex h-full max-h-[94vh] max-w-[92vw] flex-col items-center justify-center">
            <img
              ...
              width={1080}
              height={1920}
              className="min-h-0 max-h-[calc(94vh-54px)] max-w-full object-contain"
            />
            <figcaption className="mt-3 text-center font-display text-base text-white sm:text-lg">
              {selected.alt}
            </figcaption>
          </figure>
        </div>
