...
export function SeoLandingPage({ eyebrow, title, subtitle, intro, hero, gallery, featuresTitle, features, faqTitle, faqs, whatsappText, ctaLabel }: {
  eyebrow: string;
  title: string;
  subtitle: string;
  intro: string[];
  ...
  return (
    <main className="bg-white text-[#2a2f2c]">
      ...
      <section className="mx-auto grid max-w-7xl gap-10 px-4 py-20 sm:px-8 sm:py-28 lg:grid-cols-[0.8fr_1.2fr] lg:items-start">
        <p className="text-xs font-semibold uppercase tracking-[0.3em] text-primary/55">Feito para o seu espaço</p>
        <div>
          {intro.map((paragraph, index) => <p key={index} className="mb-5 text-lg leading-relaxed text-primary/75 last:mb-0">{paragraph}</p>)}
        </div>
      </section>

      <section className="mx-auto grid max-w-7xl gap-4 px-4 sm:px-8 md:grid-cols-2">
        {gallery.map((image, index) => (
          <figure key={image.src} className={index === 0 && gallery.length % 2 === 1 ? "overflow-hidden rounded-3xl md:col-span-2" : "overflow-hidden rounded-3xl"}>
            <img src={image.src} alt={image.alt} loading="lazy" className="aspect-[4/3] h-full w-full object-cover" />
          </figure>
        ))}
      </section>
      ...
      <section className="mx-auto max-w-7xl px-4 py-20 sm:px-8 sm:py-28">
        <div className="max-w-3xl">
          <p className="text-xs font-semibold uppercase tracking-[0.3em] text-primary/55">Diferenciais Arteno</p>
          <h2 className="mt-4 font-display text-4xl leading-tight text-primary sm:text-5xl">{featuresTitle}</h2>
        </div>
        <div className="mt-12 grid gap-px overflow-hidden rounded-2xl bg-primary/10 sm:grid-cols-2 lg:grid-cols-4">
          {features.map((feature) => (
            <article key={feature.title} className="bg-[#f7f6f2] p-7 sm:p-8">
              ...
              <h3 className="mt-8 font-display text-xl font-semibold text-primary">{feature.title}</h3>
      ...
      {faqs && faqs.length > 0 && (
        <section className="bg-[#f2f1ed] py-20 sm:py-28">
          <div className="mx-auto grid max-w-7xl gap-10 px-4 sm:px-8 lg:grid-cols-[0.7fr_1.3fr]">
            <div><CircleHelp className="h-7 w-7 text-primary" /><h2 className="mt-6 font-display text-4xl text-primary sm:text-5xl">{faqTitle}</h2></div>
            <div className="divide-y divide-primary/15 border-y border-primary/15">
              {faqs.map((faq, index) => (
                <details key={faq.question} className="group py-5" open={index === 0}>
                  <summary className="cursor-pointer list-none pr-8 text-base font-semibold text-primary marker:hidden">{faq.question}</summary>
                  <p className="mt-3 max-w-2xl text-sm leading-relaxed text-primary/65">{faq.answer}</p>
                </details>
              ))}
            </div>
          </div>
